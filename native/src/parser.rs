...
impl Parser {
    ...
    pub fn parse_material(&mut self) -> Option<Material> {
        ...
        if self.tokens.peek()?.token_type == format!("{:?}", TokenType::LCurly) {
            self.tokens.next();
        }
        ...
        while let Some(token) = self.tokens.peek() {
            if token.token_type == format!("{:?}", TokenType::RCurly) {
                self.tokens.next();
                break;
            }
            ...
            match token.token_type.as_str() {
                ...
                _ => {
                    ...
                    if key_token.token_type == format!("{:?}", TokenType::Identifier) {
                        self.expect(TokenType::Colon);
                        if let Some(value) = self.parse_value() {
                            material.other_properties.push((key_token.value.clone(), value));
                        }
                    } else {
                        self.tokens.next();
                    }
    ...
    fn parse_value(&mut self) -> Option<Value> {
        ...
        match token.token_type.as_str() {
            ...
            "Identifier" | "Lit" | "Unlit" | "Float" | "Sampler2d" | "Back" | "None"
            | "Opaque" | "Object" | "Uv0" | "Color" | "Position" | "Normal" => {
                let t = self.tokens.next()?;
            ...
            "LBracket" => {
                ...
                while let Some(t) = self.tokens.peek() {
                    if t.token_type == format!("{:?}", TokenType::RBracket) {
                        self.tokens.next();
                        break;
                    }
                    ...
                    if let Some(t) = self.tokens.peek() {
                        if t.token_type == format!("{:?}", TokenType::Comma) {
                            self.tokens.next();
                        }
                    }
                }
            ...
            "LCurly" => {
                ...
                let mut obj = Vec::new();
                while let Some(t) = self.tokens.peek() {
                    if t.token_type == format!("{:?}", TokenType::RCurly) {
                        self.tokens.next();
                        break;
                    }
    ...
    fn parse_object_key(&mut self) -> Option<String> {
        let token = self.tokens.peek()?;
        if token.token_type == format!("{:?}", TokenType::Identifier)
            || token.token_type == format!("{:?}", TokenType::Type)
            || token.token_type == format!("{:?}", TokenType::Parameters)
            || token.token_type == format!("{:?}", TokenType::Name)
            || token.token_type == format!("{:?}", TokenType::Requires)
            || token.token_type == format!("{:?}", TokenType::ShadingModel)
        {
    ...
    fn skip_to(&mut self, target: TokenType) {
        while let Some(token) = self.tokens.peek() {
            if token.token_type == format!("{:?}", target) {
                break;
            }
            self.tokens.next();
        }
    ...
    fn expect(&mut self, expected: TokenType) {
        if let Some(token) = self.tokens.peek() {
            if token.token_type == format!("{:?}", expected) {
                self.tokens.next();
            }
        }
    }
